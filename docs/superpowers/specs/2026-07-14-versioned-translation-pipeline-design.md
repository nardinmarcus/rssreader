...
相同 `document_hash` 的重复抽取不创建新版本。旧数据回填为 `provenance=legacy`、`raw_status=unavailable`，不得伪造原始响应；其 `document_hash` 使用 `SHA256("legacy-v1\n" + existingContentHash + documentPipelineVersions)` 确定性生成。
...
documentHash  = SHA256(rawHash + extractorVersion + sanitizerVersion + segmenterVersion)
...
- `POST /api/entry/:id/translation` 改为创建或复用任务，返回 `202`、`jobId` 和当前旧译文状态。
