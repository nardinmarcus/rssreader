...
相同文章下相同 `document_hash` 的重复抽取不创建新版本，对应数据库唯一约束为 `(entry_id, document_hash)`。旧数据回填为 `provenance=legacy`、`raw_status=unavailable`，不得伪造原始响应；旧 `content_hash` 只作为缺少原始响应时的确定性输入种子，最终 `document_hash` 仍覆盖最终 URL、当前文档流水线版本和完整语义输入，避免摘要或相对资源变化与旧不可变版本碰撞。

相对链接和图片的绝对化结果依赖最终页面 URL，因此 `final_url` 必须进入 `document_hash` 的 canonical 输入。Hacker News 等复合文章可同时包含原网页正文、提交文本、作者回复和讨论摘要：`snapshot_id` 指向主原网页快照，`source_components_json` 按稳定顺序记录其他来源组件的类型、内容哈希和可选快照 ID。完整组件记录参与 `document_hash`；`source_hash` 只使用组件类型和内容哈希，排除快照 ID、抓取时间等观察元数据。
...
用户贡献继续通过 `entry_ai_asset_contributions.id` 的稳定资产 ID 访问；其可空 `translation_version_id` 是显式最新版本指针，必须同时匹配 entry、`owner_type=user` 和 user。稳定资产 URL 跟随该用户贡献的最新版本，直接 version ID 永久定位单个历史版本。系统不得自动冒名更新用户贡献。

自动提升必须保持单调：旧 document 的同源结果只能在当前指针缺失或确实 `stale_source` 时补位；当前指针已经具有相同 `sourceHash` 后，更旧 document 的晚到结果只保存为历史。判断“新鲜站点译文”只看当前指针的 owner、`sourceHash` 和 `pipelineHash`，不能让未展示的历史系统版本阻止当前用户版本晋升。
...
标题同时作为分块上下文和稳定的 `role=title` segment：模型可借助顶层标题理解正文，但只能通过 segment 协议回传标题译文。有完整正文时，摘要只作为上下文，不重复成为必须翻译的正文段；正文缺失时摘要可成为独立 segment。
...
4. 代码段由服务端本地原样注回，不发送给模型，也不得擅自翻译代码内容；
5. 响应必须结束完整，不接受被 token 截断的 JSON；
6. 明确拒答、工具调用、内容过滤和截断结果不得发布。

### 8.3 长文分批

不再使用固定 28 块或 11,000 字符后静默截断。服务端按章节边界和任务中持久化的 `maxTokens` 输出预算划分多个 chunk，每个 chunk 带有限的文章标题和章节上下文。预算为 JSON 与不同语言 token 化保留安全余量；代码段不消耗供应商输出预算，单个可翻译 segment 预估无法放入预算时在请求前明确失败。
...
documentHash  = SHA256(canonical(primaryRawHash + sourceComponents + finalUrl + extractorVersion + sanitizerVersion + segmenterVersion + semanticInputHash))
sourceHash    = SHA256(canonical(title + translation summary context + ordered AST text + resource refs + semanticSourceComponents))
pipelineHash  = SHA256(schemaVersion + promptVersion + validationPolicyVersion)
generationHash = SHA256(documentId + sourceHash + pipelineHash + ownerPolicy + provider + model + tuning)
```

Canonical JSON 必须固定 Unicode 规范、键顺序、数组顺序和空值处理。哈希算法与 canonicalization 版本需要写入代码常量和测试夹具。

`semanticSourceComponents` 只包含稳定顺序的组件类型与内容哈希，不包含 `snapshotId`、抓取时间或响应元数据。这样原始证据变化会形成新的 `documentHash`，但只有实际翻译输入变化才会形成新的 `sourceHash`。
...
| 标题、翻译摘要上下文、正文文本、结构或资源引用改变 | 新文档；当前译文 `stale_source` | 系统译文自动排队 |
| Schema、Prompt 或验证规则不兼容升级 | `stale_pipeline` | 独立限速批次 |
| 默认模型或供应商改变 | 历史译文仍有效 | 默认否，只影响未来生成 |
| 旧记录缺少可靠哈希 | `legacy_unknown` | 不在首发自动重跑 |

`generationHash` 用于同一规范化文档内的任务幂等和结果复现；`documentId` 防止内容相同但属于不同文章的任务或版本串线。它不直接等同于“现有译文是否过期”。模型切换会产生不同 `generationHash`，但不会单独令历史译文失效。
...
任务使用 `generationHash` 唯一约束去重。相同 generation 的 `failed` 或 `superseded` 任务只有在精确 document/source 再次成为当前目标且尚无不可变版本时才可原子重开；成功 chunk 保留，未完成 chunk 重置。Worker 默认并发为 1，通过 SQLite 事务领取任务并写入租约 token 与过期时间。Worker 在每个 chunk 边界和发布前续租并校验 token；默认 240 秒租约覆盖两次 90 秒供应商尝试及安全余量。进程启动时，租约过期的 `running` 任务重新进入队列；只有 `retry_wait` 或未过期租约时，Worker 按数据库中的下一唤醒时间等待，不能依赖新的 HTTP 请求偶然唤醒。父进程异常失去 Worker 后按持久化活动任务有界重启；旧 `pipelineHash` 任务在任何供应商调用前转为 `superseded`。
...
当更新的 `sourceHash` 出现时，旧任务不再领取新 chunk，并为新文档创建任务。发布前在同一 SQLite 事务中再次检查 `entries.current_document_id` 和当前译文的语义来源：

- 仍是任务的 `document_id`：插入翻译版本并切换当前指针；
- 已切换到同 `sourceHash` 的新 document，且当前译文缺失或 `stale_source`：允许同源结果补位；
- 已有同 `sourceHash` 的当前译文，或当前 document 的 `sourceHash` 已不同：晚到结果只保存为历史，不得修改当前指针。
...
- 新增只读任务查询接口：用户任务仅所有者或管理员可见；系统任务向公共阅读器只返回 allowlist 后的状态、完成 chunk 数、总 chunk 数和通用安全错误。
- 稳定贡献 asset URL 解析到该贡献的最新用户版本；直接 `translationVersion.id` URL 定位不可变历史，不随资产头变化。

API 不返回原始 HTML 文件路径、原始响应内容、内部 Prompt 或服务端密钥。

兼容双写到旧表时，`content_hash` 继续使用旧读取器的 `SHA256(title + "\\n" + content)` 域；V2 `sourceHash` 只保存在版本化表。这样切回 `off` 不会把刚发布的 V2 译文误判为原文已更新。
...
将当前 `entries.content` 转为确定性的 `provenance=legacy` 规范化文档，设置 `raw_status=unavailable`，并让摘要上下文、最终 URL 和文档流水线版本参与身份。把现有 `entry_translations` 和用户贡献复制为不可变翻译版本，保留原作者、用户、模型、内容和时间。

旧 `entry_translations.content_hash` 与 V2 `sourceHash` 属于不同哈希域，不能通过字符串相等推断来源一致；所有历史译文统一标记 `legacy_unknown`，保留迁移前正在展示的译文，但不在迁移时自动重译。回填必须校验文章数、译文数、外键、当前指针、稳定资产头和规范化内容投影。已有指针与旧投影不一致时必须停止，禁止把任一侧静默覆盖为另一侧。

### 13.3 第三阶段：影子双写

新抓取、原文补抓和翻译同时写新旧结构，读取仍使用旧路径。同步 legacy/BYOK 保存必须在一个 SQLite 事务中重新读取当前文档与输入哈希，然后同时发布 schema-1 不可变版本、旧投影、当前指针和稳定用户资产头；文档缺失或原文已变化时整笔回滚。影子期比较：
