...
function parseOpmlCatalog(text) {
  ...
  while (cursor < body.length) {
    ...
    if (name === 'head' || name === 'body') {
      // head and body are only valid as direct children of the single opml
      // root, with head preceding body.
      if (stack.length !== 1 || stack[0] !== 'opml' || (name === 'head' && opmlBodySeen)) {
        throw catalogError('catalog-malformed', `catalog response has misplaced ${name} element`);
      }
    }
    if (name === 'body') opmlBodySeen = true;
