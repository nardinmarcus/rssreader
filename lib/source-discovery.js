...
function createSourceDiscovery({
  ...
  // attempt must not make it disappear. Cold start (never succeeded) awaits
  // one merged refresh; a stale snapshot triggers one background refresh.
  // Concurrent readers coalesce and per-account feeds are never requested.
  async function ensureSourceCatalogFresh() {
    ...
    const age = now() - Number(state.lastSuccessAt);
    if (Number.isFinite(maxAgeMs) && age > maxAgeMs && !refreshInFlight) {
