...
function normalizeProvider(provider) {
  ...
  return PROVIDERS[value] ? value : 'deepseek';
}
...
async function listModels(options = {}) {
  ...
  const models = Array.isArray(data.data)
    ...
    : [];
  return { provider: config.provider, model: config.model, models };
}
...
async function translateTitleBatch(entries, { apiKey = '', author = 'system', provider = 'deepseek', baseUrl = '', model = '' } = {}) {
  const candidates = (entries || [])
    .filter(entry => entry && entry.id && entry.title && isLikelyEnglish(entry.title))
    .slice(0, 24);
  if (!candidates.length) return { translations: [], model: getConfig({ apiKey, provider, baseUrl, model }).model };

  const config = getConfig({ apiKey, provider, baseUrl, model });
  assertConfigured(config);
...
async function translateEntry(entry, { apiKey = '', provider = 'deepseek', baseUrl = '', model = '', author = 'system', force = false } = {}) {
...
async function chatWithEntry(entry, messages, { apiKey = '', provider = 'deepseek', baseUrl = '', model = '', author = '读者', userId = null } = {}) {
  if (!entry || !entry.id) throw new Error('entry is required');
  const config = getConfig({ apiKey, provider, baseUrl, model });
  assertConfigured(config);
