...
function getEntryAssetSummaries(ids) {
  ...
  for (const row of db.prepare(`
    SELECT c.entry_id, c.id, c.author, c.body, c.model, c.created_at
    FROM commentaries c
    JOIN (
      ...
      WHERE entry_id IN (${placeholders}) AND is_public = 1
      GROUP BY entry_id
    ) latest ON latest.entry_id = c.entry_id AND latest.latest_at = c.created_at
    WHERE c.is_public = 1
  `).all(...cleanIds)) {
    setPreview(row.entry_id, 'comments', row.created_at, {
      id: row.id,
      author: row.author,
      model: row.model || '',
      text: row.body,
  ...
  for (const row of db.prepare(`
    SELECT entry_id, id, author, body, model, created_at
    FROM commentaries
    WHERE entry_id IN (${placeholders}) AND is_public = 1
    ORDER BY entry_id, created_at DESC
  `).all(...cleanIds)) {
    addItemPreview(row.entry_id, 'comments', row.created_at, {
      id: row.id,
      author: row.author,
      model: row.model || '',
      text: row.body,
