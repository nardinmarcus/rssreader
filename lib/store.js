...
function getEntryAssetSummaries(ids) {
  ...
  `).all(...cleanIds)) {
    ...
    const preview = aiAssetContributionPreview(row);
  ...
  `).all(...cleanIds)) {
    ...
    if (Array.isArray(asset.items[type])) {
      asset.items[type] = asset.items[type].map(item => ({ ...item, helpfulCount }));
    }
...
function saveTranslation(entryId, translation) {
  ...
  saveAiAssetContribution(entryId, 'translation', translation, t);
...
function getEntryAiAssetPreviews(entryId, type = '', { limit = 200 } = {}) {
  ...
  const reaction = getEntryAssetReaction(id, assetType);
  return db.prepare(`
    ...
    LIMIT ?
  `).all(id, assetType, safeLimit)
    .map(row => aiAssetContributionPreview(row, reaction.helpfulCount))
    .filter(Boolean);
}
...
function saveRewrite(entryId, rewrite) {
  ...
  saveAiAssetContribution(entryId, 'rewrite', rewrite, t);
