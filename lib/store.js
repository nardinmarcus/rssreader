...
const { createPeriodicalsModule } = require('./periodicals');
...
db.exec(`
  ...
  CREATE INDEX IF NOT EXISTS idx_source_catalog_entries_platform_name
    ON source_catalog_entries(platform, name);

  CREATE TABLE IF NOT EXISTS source_catalog_state (
    id INTEGER PRIMARY KEY CHECK(id = 1),
    feed_url TEXT NOT NULL DEFAULT '',
    last_status TEXT NOT NULL DEFAULT 'empty' CHECK(last_status IN ('empty', 'ok', 'failed')),
...
const periodicals = createPeriodicalsModule({
  db,
  mode: process.env.PERIODICALS_MODE,
  aiAdapter: request => require('./deepseek').generatePeriodicalSummary(request),
  logger: record => {
    if (typeof record === 'string') console.log(record);
    else console.warn('[periodicals]', JSON.stringify(record));
  },
});
...
function getEntriesBySourceIds(sourceIds, { limit = 5000, includeContent = true } = {}) {
  ...
  return db.prepare(`
    ...
    WHERE e.source_id IN (${placeholders})
      AND COALESCE(e.deleted_at, 0) = 0
    ORDER BY CASE WHEN e.published_ts > 0 THEN e.published_ts ELSE e.created_at END DESC,
             e.updated_at DESC
    LIMIT ?
...
function normalizeSourceCatalogStateRow(row) {
  if (!row) {
    return {
      feedUrl: '',
      lastStatus: 'empty',
  ...
  return {
    ...
    lastStatus: String(row.last_status || 'empty'),
    entryCount: Number(row.entry_count) || 0,
    duplicateCount: Number(row.duplicate_count) || 0,
    skippedCount: Number(row.skipped_count) || 0,
    lastSuccessAt: Number(row.last_success_at) || null,
    lastAttemptAt: Number(row.last_attempt_at) || null,
  };
}

// Replaces the whole catalog snapshot in one transaction. Only a fully parsed
// snapshot reaches this path, so a failed refresh can never clear old entries.
function replaceSourceCatalogSnapshot({ entries, feedUrl, duplicates = 0, skipped = 0, updatedAt, attemptedAt } = {}) {
  const rows = Array.isArray(entries) ? entries : [];
  const t = Number(updatedAt) || now();
  const attempted = Number(attemptedAt) || t;
  db.exec('BEGIN IMMEDIATE');
  try {
    db.prepare('DELETE FROM source_catalog_entries').run();
    const insert = db.prepare(`
      INSERT INTO source_catalog_entries (
        catalog_key, platform, name, feed_url, site_url, description, created_at, updated_at
      ) VALUES (?, ?, ?, ?, ?, ?, ?, ?)
    `);
    for (const entry of rows) {
      insert.run(
        ...
        String(entry.feedUrl || '').trim(),
        String(entry.siteUrl || '').trim(),
    ...
    db.prepare(`
      INSERT INTO source_catalog_state (
        id, feed_url, last_status, entry_count, duplicate_count, skipped_count,
    ...
    `).run(String(feedUrl || '').trim(), rows.length, Number(duplicates) || 0, Number(skipped) || 0, t, attempted);
    db.exec('COMMIT');
  } catch (error) {
    db.exec('ROLLBACK');
    throw error;
  }
  return getSourceCatalogState();
}

// Records a failed refresh attempt without touching the last successful snapshot.
function recordSourceCatalogRefreshFailure({ feedUrl, attemptedAt } = {}) {
  const attempted = Number(attemptedAt) || now();
  db.prepare(`
    INSERT INTO source_catalog_state (
      id, feed_url, last_status, entry_count, duplicate_count, skipped_count,
      last_success_at, last_attempt_at
    ) VALUES (1, ?, 'failed', 0, 0, 0, NULL, ?)
    ON CONFLICT(id) DO UPDATE SET
      feed_url = excluded.feed_url,
      last_status = excluded.last_status,
      last_attempt_at = excluded.last_attempt_at
  `).run(String(feedUrl || '').trim(), attempted);
  return getSourceCatalogState();
}

function getSourceCatalogState() {
  const row = db.prepare('SELECT * FROM source_catalog_state WHERE id = 1').get();
  return normalizeSourceCatalogStateRow(row);
}

function sourceCatalogFilterClause({ platform, q } = {}) {
  const conditions = [];
  const params = [];
  ...
  if (normalizedQuery) {
    conditions.push("instr(lower(name), ?) > 0");
...
function countSourceCatalogEntries(filter = {}) {
  const { where, params } = sourceCatalogFilterClause(filter);
  const row = db.prepare(`SELECT COUNT(*) AS total FROM source_catalog_entries ${where}`).get(...params);
  return Number(row.total) || 0;
}

function listSourceCatalogEntries({ platform, q, limit = 50, offset = 0 } = {}) {
  const { where, params } = sourceCatalogFilterClause({ platform, q });
  const rows = db.prepare(`
    SELECT * FROM source_catalog_entries ${where}
    ORDER BY name, catalog_key
    LIMIT ? OFFSET ?
  `).all(...params, Math.max(0, Math.min(Number(limit) || 0, 100)), Math.max(0, Number(offset) || 0));
  return rows.map(row => ({
...
module.exports = {
  ...
  countSourceCatalogEntries,
  listSourceCatalogEntries,
  recordSourceCatalogRefreshFailure,
  replaceSourceCatalogSnapshot,
  getSubmittedEntries,
  getSubmissionMeta,
