...
function getContributors({ limit = 100 } = {}) {
  ...
  return db.prepare(`
    ...
    WHERE COALESCE(c.comment_count, 0) > 0 OR COALESCE(m.chat_count, 0) > 0
    ORDER BY MAX(COALESCE(c.latest_comment_at, 0), COALESCE(m.latest_chat_at, 0)) DESC,
             (COALESCE(c.comment_count, 0) + COALESCE(m.chat_count, 0)) DESC,
  ...
  `).all(safeLimit).map(row => {
    ...
    return {
      ...
      commentCount,
      chatCount,
      assetCount: commentCount + chatCount,
      latestAt: Math.max(Number(row.latest_comment_at) || 0, Number(row.latest_chat_at) || 0),
    };
  });
}
