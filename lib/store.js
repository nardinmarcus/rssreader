...
function getEntryAssetSummaries(ids) {
  ...
  for (const row of db.prepare(`
    SELECT entry_id, content_json
    FROM entry_translations
    WHERE entry_id IN (${placeholders})
  `).all(...cleanIds)) {
    const content = safeJsonParse(row.content_json, []);
    if (out[row.entry_id]) out[row.entry_id].translation = Array.isArray(content) && content.length > 0;
  }
  ...
  `).all(...cleanIds)) {
    if (out[row.entry_id]) out[row.entry_id].rewrite = true;
  }
  ...
  for (const row of db.prepare(`
    ...
    GROUP BY entry_id
  `).all(...cleanIds)) {
    if (out[row.entry_id]) out[row.entry_id].comments = row.count || 0;
  }
  ...
  for (const row of db.prepare(`
    ...
    GROUP BY entry_id
  `).all(...cleanIds)) {
    if (out[row.entry_id]) out[row.entry_id].chatMessages = row.count || 0;
  }
...
function getChatMessages(entryId) {
  return db.prepare(`
