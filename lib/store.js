...
db.exec(`
  ...
  CREATE TABLE IF NOT EXISTS entry_asset_reactions (
    ...
    reaction TEXT NOT NULL DEFAULT 'helpful',
    created_at INTEGER NOT NULL,
    PRIMARY KEY(entry_id, asset_type, user_id, reaction),
    FOREIGN KEY(entry_id) REFERENCES entries(id) ON DELETE CASCADE,
    FOREIGN KEY(user_id) REFERENCES users(id) ON DELETE CASCADE
  );

  CREATE INDEX IF NOT EXISTS idx_entry_asset_reactions_asset
    ON entry_asset_reactions(entry_id, asset_type, reaction);
...
function getContributor(userId) {
  ...
  if (!id) return null;
  return publicContributor(db.prepare(`
...
function getContributors({ limit = 100 } = {}) {
  ...
  return db.prepare(`
    ...
    LEFT JOIN (
      SELECT user_id,
             ...
             MAX(updated_at) AS latest_translation_at
      FROM entry_translations
      WHERE user_id IS NOT NULL AND content_json IS NOT NULL AND content_json <> '' AND content_json <> '[]'
      GROUP BY user_id
    ) t ON t.user_id = u.id
    LEFT JOIN (
      SELECT user_id,
             COUNT(*) AS rewrite_count,
             MAX(updated_at) AS latest_rewrite_at
      FROM entry_rewrites
      WHERE user_id IS NOT NULL AND body IS NOT NULL AND body <> ''
      GROUP BY user_id
    ) w ON w.user_id = u.id
    LEFT JOIN (
      SELECT user_id,
             COUNT(*) AS comment_count,
             MAX(COALESCE(updated_at, created_at)) AS latest_comment_at
      FROM commentaries
    ...
    LEFT JOIN (
      SELECT user_id,
             COUNT(*) AS chat_count,
             MAX(created_at) AS latest_chat_at
      FROM chat_messages
      WHERE is_public = 1 AND user_id IS NOT NULL
      GROUP BY user_id
    ) m ON m.user_id = u.id
    ...
    ORDER BY MAX(COALESCE(t.latest_translation_at, 0), COALESCE(w.latest_rewrite_at, 0), COALESCE(c.latest_comment_at, 0), COALESCE(m.latest_chat_at, 0)) DESC,
             (COALESCE(t.translation_count, 0) + COALESCE(w.rewrite_count, 0) + COALESCE(c.comment_count, 0) + COALESCE(m.chat_count, 0)) DESC,
  ...
  `).all(safeLimit).map(row => {
    ...
    return {
      ...
      commentCount,
      chatCount,
      assetCount: translationCount + rewriteCount + commentCount + chatCount,
      latestAt: Math.max(
...
function hasEntryAsset(entryId, type) {
  ...
  if (assetType === 'translation') {
    ...
    return Array.isArray(content) && content.some(pair => pair && String(pair.target || '').trim());
  }
...
function getTranslation(entryId) {
  const row = db.prepare(`
    SELECT t.*, u.display_name AS contributor_name
...
function getRewrite(entryId) {
  const row = db.prepare(`
    SELECT r.*, u.display_name AS contributor_name
...
function getUserTranslations(userId, { limit = 100 } = {}) {
  ...
  const safeLimit = Math.max(1, Math.min(200, Number.parseInt(limit, 10) || 100));
  return db.prepare(`
    SELECT tr.entry_id, tr.title_zh, tr.summary_zh, tr.content_json, tr.model, tr.provider,
    ...
    LEFT JOIN (
      SELECT entry_id, COUNT(*) AS helpful_count
      FROM entry_asset_reactions
      WHERE reaction = 'helpful' AND asset_type = 'translation'
      GROUP BY entry_id
    ) r ON r.entry_id = tr.entry_id
    WHERE tr.user_id = ?
  ...
  `).all(id, safeLimit).map(row => {
    ...
    return {
      ...
      entryId: row.entry_id,
      contributorId: id,
      contributorName: row.contributor_name || row.created_by || '',
      author: row.created_by || row.contributor_name || '',
      titleZh: row.title_zh || '',
      summaryZh: row.summary_zh || '',
...
function getUserRewrites(userId, { limit = 100 } = {}) {
  ...
  const safeLimit = Math.max(1, Math.min(200, Number.parseInt(limit, 10) || 100));
  return db.prepare(`
    SELECT rw.entry_id, rw.title AS rewrite_title, rw.body, rw.model, rw.provider,
           rw.created_by, rw.created_at, rw.updated_at,
           e.source_id, e.title, e.link, e.published, e.published_ts,
           tr.title_zh,
           u.display_name AS contributor_name,
           COALESCE(r.helpful_count, 0) AS helpful_count
    ...
    LEFT JOIN (
      SELECT entry_id, COUNT(*) AS helpful_count
      FROM entry_asset_reactions
      WHERE reaction = 'helpful' AND asset_type = 'rewrite'
      GROUP BY entry_id
    ) r ON r.entry_id = rw.entry_id
    WHERE rw.user_id = ?
...
function getEntryAssetReaction(entryId, type, viewer = null) {
  const id = normalizeEntryId(entryId);
  const assetType = normalizeAssetReactionType(type);
  const viewerId = viewer && viewer.id ? String(viewer.id) : '';
  if (!id || !assetType) {
  ...
  const row = db.prepare(`
    SELECT
      ...
      MAX(CASE WHEN user_id = ? THEN 1 ELSE 0 END) AS helpful_by_me
    FROM entry_asset_reactions
    WHERE entry_id = ? AND asset_type = ? AND reaction = 'helpful'
  `).get(viewerId, id, assetType);
  return {
...
function setEntryAssetHelpful(entryId, type, userId, helpful = true) {
  const id = normalizeEntryId(entryId);
  const assetType = normalizeAssetReactionType(type);
  if (!id || !assetType || !hasEntryAsset(id, assetType)) return false;
  if (helpful) {
    db.prepare(`
      INSERT OR IGNORE INTO entry_asset_reactions (entry_id, asset_type, user_id, reaction, created_at)
      VALUES (?, ?, ?, 'helpful', ?)
    `).run(id, assetType, userId, now());
  } else {
    db.prepare(`
      DELETE FROM entry_asset_reactions
      WHERE entry_id = ? AND asset_type = ? AND user_id = ? AND reaction = 'helpful'
    `).run(id, assetType, userId);
  }
  const row = getEntryAssetReaction(id, assetType, { id: userId });
  return {
