...
function sourceIdsFrom(value, fallback = AUTO_REWRITE_SOURCE_IDS) {
...
async function runRefreshJob(job = {}, hooks = {}) {
  ...
  if (sourceId) {
    ...
    await fetcher.refreshAll((done, total, id) => {
  ...
  const sourceIds = sourceId && !AUTO_REWRITE_SOURCE_IDS.has(sourceId)
    ? []
    : sourceIdsFrom(sourceId ? [sourceId] : job.sourceIds);
  if (sourceIds.length) {
    if (hooks.onAutoRewriteStart) hooks.onAutoRewriteStart(sourceIds);
