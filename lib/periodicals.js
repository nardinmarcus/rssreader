...
function sourceInputIdentity({
  ...
  return computeCanonicalHash({
    ...
    identityVersion: INPUT_IDENTITY_VERSION,
    cadence: 'daily',
    periodKey,
    candidates,
    sources,
...
function fullInputIdentity({ sourceInputHash, asOfAt, scoreConfig = SCORE_CONFIG }) {
  return computeCanonicalHash({
    sourceInputHash,
    asOfAt,
...
function compileOpenDaily({
  ...
  const inputHash = fullInputIdentity({ sourceInputHash, asOfAt: timestamp, scoreConfig });
  const issue = {
    id: issueId,
    cadence: 'daily',
    periodKey: period.periodKey,
...
function snapshotOpenDailyInput({
  ...
  for (const candidate of candidates) {
    ...
    candidateSnapshot.push({
      entryId: evidence.entryId,
      contentHash: candidateInputContentHash({ source, evidence }),
      effectivePublishedAt: evidence.effectivePublishedAt,
    });
  }
...
function createPeriodicalsModule({
  ...
  function buildJobUsesCurrentIdentity(row, sourceInputHash) {
    return Boolean(row)
  ...
  function publishedIssueUsesCurrentIdentity(issue, publishedJob, sourceInputHash) {
    ...
    if (publishedJob && !buildJobUsesCurrentIdentity(publishedJob, sourceInputHash)) {
      return false;
    }
    const asOfAt = Number(publishedJob ? publishedJob.as_of_at : issue.last_built_at);
    return Number.isFinite(asOfAt)
      && issue.input_hash === fullInputIdentity({ sourceInputHash, asOfAt, scoreConfig });
  }
  ...
  async function runNextBuild({
    ...
    try {
      ...
      const frozenDailyHistory = frozenDailyHistoryFromSqlite(db, issueRow.period_key);
  ...
  function syncOpenDaily({ now, trigger = 'unspecified' } = {}) {
    ...
    try {
      ...
      const latestJobInputIsCurrent = latestJobRow
        ...
        && buildJobUsesCurrentIdentity(latestJobRow, snapshot.sourceInputHash);
