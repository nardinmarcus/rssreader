...
function createPeriodicalsModule({
  ...
  function publishedIssueUsesCurrentIdentity(
    ...
    if (publishedJob) {
      return issue.source_input_hash === sourceInputHash
        && buildJobUsesCurrentIdentity(publishedJob, sourceInputHash)
        && issue.input_hash === fullInputIdentity({ sourceInputHash, asOfAt, scoreConfig });
    }
    if (issue.source_input_hash === sourceInputHash
      && issue.input_hash === fullInputIdentity({ sourceInputHash, asOfAt, scoreConfig })) {
      return true;
    }
    return Boolean(legacySourceInputHash)
