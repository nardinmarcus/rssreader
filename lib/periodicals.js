...
function eventScore(
  ...
  const freshnessPoints = roundOne(SCORE_CONFIG.freshness.maxPoints
    * (2 ** (-exactAgeHours / SCORE_CONFIG.freshness.halfLifeHours)));
  ...
  return {
    ...
    sourceQuality: { priority, points: SCORE_CONFIG.sourceQuality[priority] },
    confirmation: {
      independentSourceCount,
      points: Math.min(
        SCORE_CONFIG.confirmation.maxPoints,
        SCORE_CONFIG.confirmation.pointsPerAdditionalSource
    ...
    persistence: {
      daysPresent,
      points: Math.min(
        SCORE_CONFIG.persistence.maxPoints,
        SCORE_CONFIG.persistence.pointsPerDay * daysPresent,
      ),
    },
    trend: {
      baselineSourceCount,
      ...
      points: baselineSourceCount > 0
        ? Math.min(
            SCORE_CONFIG.trend.maxPoints,
            SCORE_CONFIG.trend.pointsPerAdditionalSource * sourceIncrease,
          )
        : 0,
    },
    freshness: {
      ageHours,
      halfLifeHours: SCORE_CONFIG.freshness.halfLifeHours,
      points: freshnessPoints,
    },
    behavior: {
      enabled: SCORE_CONFIG.behavior.enabled,
      starredCount: 0,
      viewCount: 0,
      points: 0,
    },
...
function selectionAlgorithmContext() {
  return {
...
function selectionAlgorithmMatches(value) {
  try {
    const parsed = JSON.parse(String(value || '{}'));
    const expected = selectionAlgorithmContext();
...
function createPeriodicalsModule({ db, mode, aiAdapter, logger }) {
  ...
  async function summarizeIssue({ cadence, periodKey, now = Date.now() }) {
    if (normalizedMode === 'off') {
      const error = new Error('periodical summary requires shadow or on mode');
      error.statusCode = 409;
      throw error;
    }
    const current = getIssue({ cadence, periodKey });
    if (current.issue.status === 'frozen' || current.issue.summaryStatus === 'generated') return current;
    const summarized = await summarizePeriodicalIssue({
      ...current,
      issue: {
        ...current.issue,
        lastBuiltAt: current.generatedAt,
        frozenAt: current.frozenAt,
      },
    }, { aiAdapter, logger });
    if (summarized === current || summarized.issue.summaryStatus !== 'generated') return current;
    ...
    try {
      ...
      if (!persisted
        ...
        || persisted.content_hash !== current.issue.contentHash) {
        db.exec('ROLLBACK');
        return getIssue({ cadence, periodKey });
      }

      db.prepare(`
        UPDATE periodical_themes
        SET display_order = display_order + 1000
        WHERE issue_id = ?
      `).run(current.issue.id);
      ...
      `);
      for (const theme of summarized.themes) {
        upsertTheme.run(
          theme.id,
          current.issue.id,
          theme.themeKey,
      ...
      const updateEvent = db.prepare(`
        ...
        SET theme_id = ?, title = ?, summary = ?, summary_evidence_json = ?
        WHERE id = ? AND issue_id = ?
      `);
      for (const event of summarized.events) {
        ...
        if (result.changes !== 1) throw new Error('periodical Event changed during summary write');
      }

      if (summarized.themes.length) {
        const placeholders = summarized.themes.map(() => '?').join(', ');
        db.prepare(`
          DELETE FROM periodical_themes
          WHERE issue_id = ? AND id NOT IN (${placeholders})
        `).run(current.issue.id, ...summarized.themes.map(theme => theme.id));
      } else {
        db.prepare('DELETE FROM periodical_themes WHERE issue_id = ?').run(current.issue.id);
      }
      const issueUpdate = db.prepare(`
        UPDATE periodical_issues
        SET overview = ?, summary_version = ?, content_hash = ?,
          summary_status = ?, provider = ?, model = ?, updated_at = ?
        WHERE id = ? AND status <> 'frozen' AND content_hash = ?
      `).run(
        summarized.issue.overview,
        summarized.issue.summaryVersion,
        summarized.issue.contentHash,
        summarized.issue.summaryStatus,
        summarized.issue.provider,
        summarized.issue.model,
        Number(now),
        current.issue.id,
        current.issue.contentHash,
      );
      if (issueUpdate.changes !== 1) throw new Error('periodical Issue changed during summary write');
      db.exec('COMMIT');
    } catch (error) {
  ...
  function syncOpenDaily({ now } = {}) {
    ...
    try {
      ...
      const existing = db.prepare(`
        SELECT
          volume_no, revision, coverage_started_at, status,
          source_input_hash, selection_version, summary_version,
      ...
      if (existing
        ...
        && existing.selection_version === compiled.issue.selectionVersion
        && existing.summary_version === compiled.issue.summaryVersion
        && selectionAlgorithmMatches(existing.selection_context_json)) {
        db.exec('COMMIT');
