...
function createPeriodicalsModule({
  ...
  function listIssues({ cadence, cursor, limit: requestedLimit }) {
    ...
    return readPeriodicalData(() => {
      ...
      while (visibleRows.length <= limit) {
        ...
        const nextScanCursor = rows[rows.length - 1].period_key;
        if (nextScanCursor === scanCursor) break;
  ...
  function monthlyIssueIdentityCode(issueValue, eligibleStartAt) {
    ...
    if (expectedVolume === null || Number(issue.volume_no) !== expectedVolume) {
      return 'volume_order';
    }
    return null;
  }
  ...
  function frozenMonthlyValidationCode(issueValue, storedValue = null) {
    try {
      ...
      if (issue.id !== `periodical:monthly:${period.periodKey}`
        ...
        || !String(issue.selection_version || '').trim()
        || !String(issue.summary_version || '').trim()
      ...
      const succeededJob = db.prepare(`
        ...
        WHERE issue_id = ? AND status = 'succeeded'
          ...
          AND selection_version = ? AND summary_version = ?
        LIMIT 1
      `).get(
        issue.id,
        issue.source_input_hash,
        issue.input_hash,
        issue.selection_version,
        issue.summary_version,
      );
  ...
  function reconcileMonthlyDrafts({ eligibleStartAt, latest, timestamp, triggerReason }) {
    ...
    try {
      ...
      for (const issue of frozen) {
        const validationCode = monthlyIssueIdentityCode(issue, eligibleStartAt);
  ...
  function syncMonthlyRollup({ now, trigger = 'monthly-sweep' } = {}) {
    ...
    if (blockers.length) return { ...blockers[0], issues: blockers };

    const issues = due.periods.map(target => syncRollupPeriod({
      cadence: 'monthly',
      timestamp,
      triggerReason,
      target,
