...
const WEEKLY_INPUT_RETRY_MS = 5 * MINUTE_MS;
...
function pendingWeeklyInputs(errors = []) {
  const error = new Error('Weekly inputs are not complete Frozen Dailies');
  error.code = 'ERR_PERIODICAL_WEEKLY_INPUTS_PENDING';
  error.statusCode = 425;
  error.inputErrors = errors;
  return error;
}

function weeklyEvidenceSnapshot(evidence) {
  return {
...
function weeklyDailyValidationCode(document) {
...
function conflictingWeeklyEvidenceDailyIds(documents) {
  const byEntryId = new Map();
  const conflicts = new Set();
  for (const document of documents) {
    for (const evidence of document.evidence) {
      const snapshot = weeklyEvidenceSnapshot(evidence);
...
function validateCompiledWeekly(compiled, { job, issue, expectedInputs }) {
  if (!compiled || typeof compiled !== 'object') throw invalidBuild('compiled issue is missing');
  if (!compiled.issue || !Array.isArray(compiled.themes)
    || !Array.isArray(compiled.events) || !Array.isArray(compiled.evidence)
    || !Array.isArray(compiled.inputs)) {
    throw invalidBuild('compiled Weekly is incomplete');
  }
  const document = compiled.issue;
  if (document.id !== issue.id || document.cadence !== 'weekly'
    || document.periodKey !== issue.period_key
    || !['finalizing', 'frozen'].includes(document.status)) {
    throw invalidBuild('compiled Weekly identity is invalid');
  }
  if (document.status === 'frozen' && !Number.isFinite(Number(document.frozenAt))) {
    throw invalidBuild('compiled frozen Weekly identity is invalid');
  }
  if (document.sourceInputHash !== job.sourceInputHash
    || document.inputHash !== job.inputHash
    || document.selectionVersion !== WEEKLY_SELECTION_VERSION
    || document.summaryVersion !== SUMMARY_VERSION) {
    throw invalidBuild('compiled Weekly input identity is invalid');
  }
  if (document.revision !== Number(issue.revision) + 1
    || document.volumeNo !== Number(issue.volume_no)) {
    throw invalidBuild('compiled Weekly revision identity is invalid');
  }
  if (compiled.inputs.length !== 7
    || canonicalSerialize(compiled.inputs) !== canonicalSerialize(expectedInputs)) {
    throw invalidBuild('compiled Weekly input chain is invalid');
  }
  ...
  if (document.contentHash !== computePeriodicalContentHash(compiled)) {
    throw invalidBuild('compiled Weekly content hash is invalid');
  }
  return compiled;
}
...
function weeklyRollupScore(dailyScores, daysPresent, distinctSources) {
  ...
  const exactTop3Mean = top3.length
    ...
    : 0;
  const rawPoints = {
    maximum: WEEKLY_SCORE_CONFIG.maxDailyScoreWeight * maxDailyScore,
    top3: WEEKLY_SCORE_CONFIG.meanTop3DailyScoresWeight * exactTop3Mean,
    occurrence: WEEKLY_SCORE_CONFIG.occurrenceDayPoints
      * Math.min(1, Math.max(0, daysPresent - 1) / 6),
    breadth: WEEKLY_SCORE_CONFIG.sourceBreadthPoints
  ...
  return {
    ...
    score: {
      ...
      maxDailyScore: {
        value: maxDailyScore,
        weight: WEEKLY_SCORE_CONFIG.maxDailyScoreWeight,
        points: roundOne(rawPoints.maximum),
      },
      meanTop3DailyScores: {
        value: roundOne(exactTop3Mean),
        weight: WEEKLY_SCORE_CONFIG.meanTop3DailyScoresWeight,
        points: roundOne(rawPoints.top3),
      },
      occurrenceDays: { daysPresent, periodDays: 7, points: roundOne(rawPoints.occurrence) },
...
function compileWeekly({
  ...
  const timestamp = Number(now);
  if (!Number.isFinite(timestamp)) throw new TypeError('now must be a finite timestamp');
  const period = shanghaiIsoWeekPeriodFromKey(periodKey);
  ...
  if (orderedDailies.length !== 7) throw invalidBuild('weekly build requires seven Dailies');
  const issueId = `periodical:weekly:${period.periodKey}`;
  const dailyEvents = [];
  ...
  const compiledEvents = eventClusters.map(group => {
    ...
    for (const item of group) {
      for (const dailyEvidence of item.evidence) {
        const snapshot = weeklyEvidenceSnapshot(dailyEvidence);
    ...
    const { importanceScore, score } = weeklyRollupScore(
    ...
    return {
      ...
      event: {
        ...
        summary: boundedFallbackSummary([
          ...
          `该事件本周出现 ${daysPresent} 天，覆盖 ${distinctSources} 个来源。`,
        ].filter(Boolean).join(' ')),
        summaryEvidenceIds,
        whySelected: [
          `最高日报重要性 ${score.maxDailyScore.value} 分`,
          `top-3 日报均值 ${score.meanTop3DailyScores.value} 分`,
          `本周出现 ${score.occurrenceDays.daysPresent} 天`,
  ...
  const selected = compiledEvents
    .filter(item => item.event.importanceScore >= WEEKLY_SCORE_CONFIG.threshold)
    .sort((left, right) => (right.event.importanceScore - left.event.importanceScore)
      ...
      || (right.event.effectiveAt - left.event.effectiveAt)
      || left.event.eventKey.localeCompare(right.event.eventKey))
    .slice(0, WEEKLY_SCORE_CONFIG.maxEvents);
  const themes = [];
  const themeMap = new Map();
  selected.forEach((item, displayOrder) => {
    item.event.displayOrder = displayOrder;
  ...
  for (const theme of themes) {
    ...
    theme.trendNote = `本周该主题收录 ${themeEvents.length} 个事件，其中 ${recurring} 个跨日出现。`;
  }
  const events = selected.map(item => item.event);
  const evidence = selected.flatMap(item => item.evidence);
  const overview = events.length
    ? `本周从七份冻结日报汇总出 ${events.length} 个达到 45 分门槛的事件，分布于 ${themes.length} 个主题。所有证据均复制自冻结日报快照，并按最高日报分、top-3 均值、出现天数和来源广度确定性排序。`
    : '本周七份冻结日报均已完整覆盖，但没有事件达到 45 分入选门槛。所有输入日报与内容哈希已按日期顺序固化。';
...
function createPeriodicalsModule({
  ...
  function listIssues({ cadence, cursor, limit: requestedLimit }) {
    ...
    return readPeriodicalData(() => {
      const rows = db.prepare(`
        ...
        WHERE issue.cadence = ?
          AND (? <> 'weekly' OR issue.status = 'frozen')
  ...
  function snapshotWeeklyInput(periodKey) {
    const period = shanghaiIsoWeekPeriodFromKey(periodKey);
    const issueId = `periodical:weekly:${period.periodKey}`;
    const inputStates = [];
    const documents = [];
    const inputs = [];
    const errors = [];
    for (let displayOrder = 0; displayOrder < 7; displayOrder += 1) {
      ...
      if (!row) validationCode = 'missing';
        ...
        try {
          ...
          if (Number(document.issue.revision) < 1
            ...
            validationCode = weeklyDailyValidationCode(document);
    ...
    const conflictingDailyIds = conflictingWeeklyEvidenceDailyIds(documents);
    ...
    return {
      valid: errors.length === 0 && documents.length === 7,
      errors,
      period,
      issueId,
      inputStates,
  ...
  function firstEligibleWeeklyStart() {
    const firstDaily = db.prepare(`
      SELECT period_key, period_start_at
      FROM periodical_issues
      WHERE cadence = 'daily'
    ...
    const firstWeek = shanghaiIsoWeekPeriod(Number(firstDaily.period_start_at));
    const firstMondayKey = shanghaiDailyPeriod(firstWeek.periodStartAt).periodKey;
    return firstDaily.period_key === firstMondayKey
      ? firstWeek.periodStartAt
      : firstWeek.periodEndAt;
  }
  ...
  function getIssue({ cadence, periodKey }) {
    ...
    return readPeriodicalData(() => readIssueSnapshot(() => {
      ...
      if (!issue || (normalizedCadence === 'weekly' && issue.status !== 'frozen')) {
  ...
  function finishBuildError(jobId, leaseToken, error, timestamp) {
    ...
    const waitsForWeeklyInputs = error
      && error.code === 'ERR_PERIODICAL_WEEKLY_INPUTS_PENDING';
    const retry = waitsForWeeklyInputs
      || (retryableBuildError(error) && job.attemptCount < MAX_BUILD_ATTEMPTS);
    const nextRetryAt = retry
      ? timestamp + (waitsForWeeklyInputs
          ? WEEKLY_INPUT_RETRY_MS
  ...
  function publishClaimedBuild({ job, compiled, leaseToken, timestamp }) {
    ...
    try {
      ...
      const currentInput = issueRow.cadence === 'weekly'
        ? snapshotWeeklyInput(issueRow.period_key)
      ...
      const currentIdentityMatches = issueRow.cadence === 'weekly'
        ? currentInput.valid
      ...
      if (issueRow.cadence === 'weekly') {
        validateCompiledWeekly(compiled, {
          job,
          issue: issueRow,
  ...
  async function runNextBuild({
    ...
    try {
      const issueRow = db.prepare(`
        SELECT * FROM periodical_issues WHERE id = ?
      `).get(job.issueId);
      const weeklyBuild = issueRow && issueRow.cadence === 'weekly';
      const expectedSelectionVersion = weeklyBuild
        ? WEEKLY_SELECTION_VERSION
        : SELECTION_VERSION;
      const expectedScoreConfig = weeklyBuild ? WEEKLY_SCORE_CONFIG : scoreConfig;
      if (!issueRow || !['open', 'finalizing'].includes(issueRow.status)
        || !['daily', 'weekly'].includes(issueRow.cadence)
      ...
      const buildIdentityMatches = weeklyBuild
      ...
      if (!buildIdentityMatches) {
        ...
        if (weeklyBuild) {
          const replacement = syncWeeklyPeriod({
            timestamp: inputChangedAt,
            triggerReason: 'weekly-input-changed',
            target: shanghaiIsoWeekPeriodFromKey(issueRow.period_key),
      ...
      if (weeklyBuild && !buildInput.valid) throw pendingWeeklyInputs(buildInput.errors);
      const compiler = compileIssue || (weeklyBuild ? compileWeekly : compileOpenDaily);
      compiled = weeklyBuild
        ? await compiler({
            now: job.asOfAt,
            periodKey: issueRow.period_key,
      ...
      const dailyFinalization = !weeklyBuild && issueRow.status === 'finalizing';
  ...
  function syncWeeklyPeriod({ timestamp, triggerReason, target, staleClaim = null }) {
    let result;

    db.exec('BEGIN IMMEDIATE');
    try {
      const eligibleStartAt = firstEligibleWeeklyStart();
      if (eligibleStartAt === null || target.periodStartAt < eligibleStartAt) {
        db.exec('COMMIT');
        return {
          action: 'skipped-partial-week',
          issueId: null,
          periodKey: target.periodKey,
          job: null,
        };
      }
      const snapshot = snapshotWeeklyInput(target.periodKey);
      let issue = db.prepare('SELECT * FROM periodical_issues WHERE id = ?')
      ...
      const nextRetryAt = snapshot.valid ? null : timestamp + WEEKLY_INPUT_RETRY_MS;
      const errorCode = snapshot.valid ? null : 'ERR_PERIODICAL_WEEKLY_INPUTS_PENDING';
      let action = 'noop';
      if (job && ['failed', 'superseded'].includes(job.status)) {
        db.prepare(`
          UPDATE periodical_build_jobs
        ...
        `).run(
          ...
          target.periodEndAt,
          WEEKLY_SELECTION_VERSION,
          canonicalSerialize(WEEKLY_SCORE_CONFIG),
          SUMMARY_VERSION,
          triggerReason,
          desiredStatus,
          nextRetryAt,
    ...
    safeBuildLog(logger, {
      ...
      eventCount: 0,
      state: result.action === 'queued' ? 'queued' : result.action,
      durationMs: 0,
    });
    return result;
  }
