...
function createPeriodicalsModule({
  ...
  function oldestDueMonthlyPeriod(timestamp) {
    const latest = previousFullShanghaiNaturalMonth(timestamp);
    const eligibleStartAt = firstEligibleMonthlyStart();
    if (eligibleStartAt === null || latest.periodStartAt < eligibleStartAt) return latest;
    ...
    while (target.periodStartAt <= latest.periodStartAt) {
      const issue = findIssue.get(target.periodKey);
      if (!issue || issue.status !== 'frozen') return target;
  ...
  function syncMonthlyRollup({ now, trigger = 'monthly-sweep' } = {}) {
    ...
    return syncRollupPeriod({
      ...
      timestamp,
      triggerReason,
      target: oldestDueMonthlyPeriod(timestamp),
    });
  }
