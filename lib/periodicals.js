...
function compileOpenDaily({
  ...
  const eligibleCandidates = compiledCandidates
    .map(item => {
      const input = candidateInputSnapshot({ source: item.source, evidence: item.evidence });
      return {
  ...
  const sourceInputHash = sourceInputIdentity({
    ...
    candidates: eligibleCandidates.map(candidateIdentitySnapshot),
...
function snapshotOpenDailyInput({
  ...
  for (const candidate of candidates) {
    ...
    const inputSnapshot = candidateInputSnapshot(input);
    candidateSnapshot.push({
      ...identity,
      sourceId: evidence.sourceId,
      contentHash: candidateInputSnapshotHash(inputSnapshot),
      input: inputSnapshot,
    });
