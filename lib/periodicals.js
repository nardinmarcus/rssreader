...
const { getPeriodicalsStatus } = require('./periodicals-status');
const {
  computeRevisionContentHash,
  freezeCompiledIssue,
  revisionContentHash,
} = require('./periodical-revision');
const {
  SUMMARY_VERSION,
...
const CANDIDATE_SNAPSHOT_VERSION = 'periodical-candidate-v2';
const LEGACY_CANDIDATE_SNAPSHOT_VERSION = 'periodical-candidate-v1';
...
function plainTextExcerpt(...values) {
  ...
  return characters.length <= 280 ? text : `${characters.slice(0, 279).join('').trimEnd()}…`;
}
...
function candidateInputSnapshotHash(snapshot) {
  return computeCanonicalHash(snapshot);
}

function candidateInputContentHash(input) {
  return candidateInputSnapshotHash(candidateInputSnapshot(input));
}

function candidateIdentitySnapshot(candidate) {
  return {
    entryId: candidate.entryId,
    contentHash: candidate.contentHash,
    effectivePublishedAt: candidate.effectivePublishedAt,
  };
}
...
function buildSourceSnapshot(source) {
  return {
    ...
    name: String(source && source.name || source && source.id || ''),
    category: String(source && source.category || ''),
    enabled: true,
    editorialPriority: normalizedPriority(source && source.editorialPriority),
    labels: normalizedLabels(source && source.labels),
  };
}

function sourceIdentitySnapshot(sourceSnapshot) {
  return {
    sourceId: sourceSnapshot.sourceId,
    enabled: Boolean(sourceSnapshot.enabled),
    editorialPriority: sourceSnapshot.editorialPriority,
...
function legacyCandidateInputContentHash(input) {
  return computeCanonicalHash({
    version: LEGACY_CANDIDATE_SNAPSHOT_VERSION,
...
function periodicalBuildJobId({ issueId, inputHash, summaryVersion = SUMMARY_VERSION }) {
  return `periodical-job:${computeCanonicalHash({
    issueId,
    inputHash,
    summaryVersion,
  })}`;
...
function dailySelectionContext({
  scoreConfig = SCORE_CONFIG,
  candidateSnapshot = [],
  sourceSnapshot = [],
  frozenDailyHistory = [],
...
function legacySelectionAlgorithmMatches(value) {
  const expected = {
    canonicalizationVersion: CANONICALIZATION_VERSION,
    candidateSnapshotVersion: LEGACY_CANDIDATE_SNAPSHOT_VERSION,
    urlCanonicalizationVersion: URL_CANONICALIZATION_VERSION,
...
function rollupSelectionContext(cadence, dailyInputCount) {
...
function rollupInputIdentity({ cadence, periodKey, inputStates }) {
  const config = rollupConfig(cadence);
  const inputs = Array.isArray(inputStates) ? inputStates : [];
  ...
  return {
    sourceInputHash,
    inputHash,
    selectionContext: rollupSelectionContext(cadence, inputs.length),
    selectionVersion: config.selectionVersion,
    summaryVersion: SUMMARY_VERSION,
  };
}
...
function validateCompiledOpenDaily(compiled, { job, issue }) {
  ...
  const expectedContentHash = computeRevisionContentHash(compiled);
...
function validateCompiledRollup(compiled, {
  ...
  if (document.contentHash !== computeRevisionContentHash(compiled)) {
    throw invalidBuild('compiled rollup content hash is invalid');
  }
  return compiled;
}

// Internal Daily Input Snapshot seam shared by compilation and SQLite reads.
// `now` interprets future timestamps; the cutoff independently admits candidates.
// Each caller retains its clock policy, including finalization's scoring time.
function prepareDailyInputSnapshot({
  period,
  now,
  ...
  for (const candidate of candidates) {
    ...
    const evidence = candidateEvidence({ candidate, source, now, eventId: '' });
    if (Number(candidate.createdAt) > candidateCutoffAt
      || evidence.effectivePublishedAt < period.periodStartAt
      || evidence.effectivePublishedAt >= period.periodEndAt) continue;
    preparedCandidates.push({ candidate, source, evidence });
    const input = candidateInputSnapshot({ source, evidence });
    candidateSnapshot.push({
      entryId: evidence.entryId,
      sourceId: evidence.sourceId,
      contentHash: candidateInputSnapshotHash(input),
      effectivePublishedAt: evidence.effectivePublishedAt,
      input,
    });
  }
...
function compileOpenDaily({
  ...
  const snapshot = prepareDailyInputSnapshot({
    period,
    now: timestamp,
  ...
  const { scoreConfig, sourceInputHash, selectionContext } = snapshot;
  const history = snapshot.frozenDailyHistory;
  const compiledCandidates = snapshot.preparedCandidates.map(({ candidate, source, evidence }) => ({
    candidate,
    source,
    evidence,
  ...
  const identity = dailyInputIdentity({
    sourceInputHash,
    asOfAt: timestamp,
  ...
  issue.contentHash = computeRevisionContentHash({ issue, themes, events, evidence });
...
function compileRollup({
  ...
  issue.contentHash = computeRevisionContentHash({ issue, themes, events, evidence, inputs });
...
function snapshotOpenDailyInput({
  ...
  const period = periodKey
    ? shanghaiDailyPeriodFromKey(periodKey)
    : shanghaiDailyPeriod(timestamp);
  const frozenDailyHistory = frozenDailyHistoryFromSqlite(db, period.periodKey);
  ...
  // Legacy identity is a scheduling compatibility policy, not compiler input.
  const legacySourceInputHash = behaviorSignalEnabled
    ? null
    : legacySourceInputIdentity({
      periodKey: period.periodKey,
      candidates: snapshot.preparedCandidates.map(({ source, evidence }) => ({
        entryId: evidence.entryId,
        effectivePublishedAt: evidence.effectivePublishedAt,
        contentHash: legacyCandidateInputContentHash({ source, evidence }),
      })).sort((left, right) => left.entryId.localeCompare(right.entryId)),
      sources: snapshot.sourceSnapshot.map(sourceIdentitySnapshot),
    });
  return { ...snapshot, period, sources, candidates, legacySourceInputHash };
}
...
const MONTHLY_SCAN_CURSOR_PREFIX = 'monthly-scan-v1.';
...
function encodeMonthlyScanCursor(periodKey) {
  const rawPeriodKey = String(periodKey);
  return validPeriodKey('monthly', rawPeriodKey)
    ? rawPeriodKey
    : `${MONTHLY_SCAN_CURSOR_PREFIX}${Buffer.from(rawPeriodKey).toString('base64url')}`;
}

function decodeMonthlyScanCursor(value) {
  const cursor = String(value || '');
  if (validPeriodKey('monthly', cursor)) return cursor;
  if (!cursor.startsWith(MONTHLY_SCAN_CURSOR_PREFIX)) return null;
  const encoded = cursor.slice(MONTHLY_SCAN_CURSOR_PREFIX.length);
  if (!/^[A-Za-z0-9_-]+$/.test(encoded)) return null;
  try {
    const decoded = Buffer.from(encoded, 'base64url').toString();
    return Buffer.from(decoded).toString('base64url') === encoded ? decoded : null;
  } catch {
    return null;
  }
}
...
function readStoredPeriodicalIssue(db, issue) {
...
function createPeriodicalsModule({
  ...
  function getAdminStatus(options = {}) {
    return readPeriodicalData(() => getPeriodicalsStatus(db, {
      ...options,
      mode: normalizedMode,
    }));
  }
  ...
  function listIssues({ cadence, cursor, limit: requestedLimit }) {
    ...
    if (normalizedCursor !== null) {
      scanCursor = normalizedCadence === 'monthly'
        ? decodeMonthlyScanCursor(normalizedCursor)
        : (validPeriodKey(normalizedCadence, normalizedCursor) ? normalizedCursor : null);
      if (scanCursor === null) throw invalidPeriodicalRequest('invalid cursor');
    }
    ...
    return readPeriodicalData(() => {
      ...
      const batchLimit = normalizedCadence === 'monthly'
        ? Math.min(100, Math.max(30, limit))
        : limit + 1;
      const visibleRows = [];
      ...
      for (const row of rows) {
        ...
        if (visibleRows.length >= limit && normalizedCadence === 'monthly') break;
        if (visibleRows.length > limit) break;
      }
      const hasMoreVisible = visibleRows.length > limit;
      const page = hasMoreVisible ? visibleRows.slice(0, limit) : visibleRows;
      const monthlyHasContinuation = normalizedCadence === 'monthly'
        && lastScannedPeriodKey !== null
        && (scannedRowCount < rows.length || rows.length === batchLimit);
      return {
        issues: page.map(row => ({
          cadence: row.cadence,
        ...
        nextCursor: normalizedCadence === 'monthly'
          ? (monthlyHasContinuation ? encodeMonthlyScanCursor(lastScannedPeriodKey) : null)
          : (hasMoreVisible ? page[page.length - 1].period_key : null),
      };
    });
  }

  function storedIssueDocument(issue) {
    return readStoredPeriodicalIssue(db, issue);
  }
  ...
  function snapshotRollupInput(cadence, periodKey) {
    ...
    for (let displayOrder = 0; displayOrder < periodDays; displayOrder += 1) {
      ...
      if (!row) validationCode = 'missing';
        ...
        try {
          ...
          if (Number(document.issue.revision) < 1
            || revisionContentHash(document) !== row.content_hash) {
      ...
      inputStates.push({
        ...
        actualRevision: row ? Number(row.revision) : null,
        dailyContentHash,
        validationCode,
      });
    }
  ...
  function frozenMonthlyValidationCode(issueValue, storedValue = null) {
    try {
      ...
      const expectedJobId = periodicalBuildJobId({
        issueId: issue.id,
        inputHash: issue.input_hash,
        summaryVersion: SUMMARY_VERSION,
      });
      ...
      if (!/^[a-f0-9]{64}$/.test(String(issue.content_hash || ''))
        || revisionContentHash(stored) !== issue.content_hash) {
  ...
  function getEvidenceAvailability(identity) {
    const issue = getIssue(identity).issue;
    return readPeriodicalData(() => ({
      evidence: db.prepare(`
        SELECT
          evidence.event_id,
          evidence.entry_id,
          CASE
            WHEN current_entry.id IS NOT NULL
              AND COALESCE(current_entry.deleted_at, 0) = 0
            THEN 1
            ELSE 0
          END AS entry_available
        FROM periodical_event_evidence AS evidence
        INNER JOIN periodical_events AS event ON event.id = evidence.event_id
        LEFT JOIN entries AS current_entry ON current_entry.id = evidence.entry_id
        WHERE event.issue_id = ?
        ORDER BY event.display_order, evidence.display_order, evidence.entry_id
      `).all(issue.id).map(row => ({
        eventId: row.event_id,
        entryId: row.entry_id,
        entryAvailable: Boolean(row.entry_available),
      })),
    }));
  }
  ...
  function replaceCompiledIssueRows(compiled, issueRow, timestamp) {
    ...
    if (freezing) {
      ...
      `).get(issue.id));
      if (revisionContentHash(persisted) !== issue.contentHash) {
  ...
  async function runNextBuild({
    ...
    try {
      ...
      if (issueRow.status === 'finalizing') {
        compiled = freezeCompiledIssue(compiled, publishingAt);
      }
      if (rollupBuild) {
        validateCompiledRollup(compiled, {
          job,
...
module.exports = {
  ...
  eventMergeDecision,
  finalizationWindowMs: FINALIZATION_WINDOW_MS,
  fullInputIdentity,
  ...
  rollupInputIdentity,
  rollupSelectionContext,
  scoreConfigFor,
  scoringHistoryIdentity,
  selectionAlgorithmContext,
  shanghaiDailyPeriod,
