...
function eventScore(
  ...
  return {
    ...
    confirmation: {
      ...
      points: Math.min(25, 8 * Math.max(0, independentSourceCount - 1)),
    },
    persistence: { daysPresent, points: Math.min(14, 3.5 * daysPresent) },
    trend: {
      baselineSourceCount,
      sourceIncrease,
      points: baselineSourceCount > 0 ? Math.min(6, 2 * sourceIncrease) : 0,
    },
    freshness: {
      ageHours,
...
function whySelected(score) {
  ...
  if (score.persistence.points > 0) {
    reasons.push(`相关主题过去 7 个冻结日报出现 ${score.persistence.daysPresent} 天（${score.persistence.points} 分）`);
  }
  if (score.trend.points > 0) {
    reasons.push(`独立来源较近 7 日单日峰值增加 ${score.trend.sourceIncrease} 个（${score.trend.points} 分）`);
  }
