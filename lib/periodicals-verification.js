...
const { revisionContentHash } = require('./periodical-revision');
const {
  ...
  try {
    ...
    for (let displayOrder = 0; displayOrder < expectedCount; displayOrder += 1) {
      ...
      if (!daily || !daily.issue
        ...
        || revisionContentHash(daily) !== daily.issue.contentHash) {
...
function verifyEvidenceProvenance(db) {
  ...
  for (const issue of issueRows) {
    try {
      ...
      if (revisionContentHash(document) !== issue.content_hash) {
