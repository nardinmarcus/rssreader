...
function moveSourceInScope(sources = [], sourceId, direction, scope = 'category') {
  ...
  const scopedSources = scope === 'all'
    ? ordered
    : ordered.filter(item => item.category === source.category);
  const scopedIndex = scopedSources.findIndex(item => item.id === sourceId);
  const neighbor = scopedSources[scopedIndex + (direction === 'up' ? -1 : 1)];
...
function moveSourceInOrder(sources = [], sourceId, direction) {
  return moveSourceInScope(sources, sourceId, direction, 'all');
}

function moveSourceWithinCategory(sources = [], sourceId, direction) {
  return moveSourceInScope(sources, sourceId, direction, 'category');
}
