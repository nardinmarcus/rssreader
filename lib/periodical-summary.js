...
// Hash computation and published-revision semantics live in the revision
// module; this module owns constrained AI expression only.
const computePeriodicalContentHash = computeRevisionContentHash;
...
function applyGeneratedSummary(issue, output, response) {
  ...
  return sealRevisionContentHash(generated);
}
...
async function summarizePeriodicalIssue(issue, { aiAdapter, beforeAttempt, logger } = {}) {
  ...
  for (let attempt = 0; attempt < 2; attempt += 1) {
    if (typeof beforeAttempt === 'function'
      && await beforeAttempt(attempt + 1) === false) break;
    let response;
    try {
