...
test('reader exposes a retryable original-content recovery state', () => {
  ...
  const styles = fs.readFileSync(path.join(projectDir, 'public', 'styles.css'));
  const stylesVersion = crypto.createHash('sha256').update(styles).digest('hex').slice(0, 12);
  ...
  assert.match(html, new RegExp(`<link rel="stylesheet" href="/styles\\.css\\?v=${stylesVersion}" />`));
